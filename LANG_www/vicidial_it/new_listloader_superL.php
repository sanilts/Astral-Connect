...
echo "<!-- SEED TIME  $secX:   $year-$mon-$mday $hour:$min:$sec  LOCAL FUSO ORARIO ATTUALE: $LOCAL_GMT_OFF  DST: $isdst -->\n";
...
function ShowProgress(good, bad, total, dup, post) {
	...
	parent.lead_count.document.write('<html><body><table border=0 width=200 cellpadding=10 cellspacing=0 align=center valign=top><tr bgcolor="#000000"><th colspan=2><font face="arial, helvetica" size=3 color=white>Stato attuale del file:</font></th></tr><tr bgcolor="#009900"><td align=right><font face="arial, helvetica" size=2 color=white><B>Good:</B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B>'+good+'</B></font></td></tr><tr bgcolor="#990000"><td align=right><font face="arial, helvetica" size=2 color=white><B>Bad:</B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B>'+bad+'</B></font></td></tr><tr bgcolor="#000099"><td align=right><font face="arial, helvetica" size=2 color=white><B>Total:</B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B>'+total+'</B></font></td></tr><tr bgcolor="#009900"><td align=right><font face="arial, helvetica" size=2 color=white><B> &nbsp; </B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B> &nbsp; </B></font></td></tr><tr bgcolor="#009900"><td align=right><font face="arial, helvetica" size=2 color=white><B>Duplicate:</B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B>'+dup+'</B></font></td></tr><tr bgcolor="#009900"><td align=right><font face="arial, helvetica" size=2 color=white><B>Postal Match:</B></font></td><td align=left><font face="arial, helvetica" size=2 color=white><B>'+post+'</B></font></td></tr></table><body></html>');
...
<table align=center width="700" border=0 cellpadding=5 cellspacing=0 bgcolor=#D9E6FE>
  <tr>
	<td align=right width="35%"><B><font face="arial, helvetica" size=2>Carica Contatti Da Questo File:</font></B></td>
	<td align=left width="65%"><input type=file name="leadfile" value="<?php echo $leadfile ?>"> <?php echo "$NWB#vicidial_list_loader$NWE"; ?></td>
  </tr>
  <tr>
	<td align=right width="25%"><font face="arial, helvetica" size=2>Sovrascrittura ID Lista: </font></td>
	<td align=left width="75%"><font face="arial, helvetica" size=1><input type=text value="<?php echo $list_id_override ?>" name='list_id_override' size=10 maxlength=8> (solo cifre o lasciare in bianco per usare i valori contenuti nel file)</td>
  </tr>
  <tr>
	<td align=right width="25%"><font face="arial, helvetica" size=2>Sovrascrittura Codice Telefono: </font></td>
	<td align=left width="75%"><font face="arial, helvetica" size=1><input type=text value="<?php echo $phone_code_override ?>" name='phone_code_override' size=8 maxlength=6> (solo cifre o lasciare in bianco per usare i valori contenuti nel file)</td>
  </tr>
  <tr>
	<td align=right><B><font face="arial, helvetica" size=2>Struttura del file da usare:</font></B></td>
	<td align=left><font face="arial, helvetica" size=2><input type=radio name="file_layout" value="standard" checked>Standard Format&nbsp;&nbsp;&nbsp;&nbsp;<input type=radio name="file_layout" value="custom">Struttura personalizzata</td>
  </tr>
    <tr>
	<td align=right width="25%"><font face="arial, helvetica" size=2>Controlla Contatti Duplicati:</font></td>
	<td align=left width="75%"><font face="arial, helvetica" size=1><select size=1 name=dupcheck>
	<option selected value="NONE">NON CONTROLLARE DUPLICATI</option>
	<option value="DUPLIST">CONTROLLA DUPLICATI PER TELEFONO SOLO CON QUESTO ID LISTA</option>
	<option value="DUPCAMP">CONTROLLA DUPLICATI PER TELEFONO IN TUTTE LE LISTE DELLA CAMPAGNA</option>
	<option value="DUPSYS">CONTROLLA DUPLICATI PER TELEFONO NELL` INTERO SISTEMA</option>
	<option value="DUPTITLEALTPHONELIST">CHECK FOR DUPLICATES BY TITLE/ALT-PHONE IN ID LISTA</option>
	<option value="DUPTITLEALTPHONESYS">CHECK FOR DUPLICATES BY TITLE/ALT-PHONE IN ENTIRE SYSTEM</option>
	</select></td>
  </tr>
  <tr>
	<td align=right width="25%"><font face="arial, helvetica" size=2>Ricerca Fusi Orari (Solo USA): </font></td>
...
<tr>
	<td align=center colspan=2><input type=submit value="INVIA" name='submit_file'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=button onClick="javascript:document.location='new_listloader_superL.php'" value="RICOMINCIA" name='reload_page'></td>
  </tr>
  <tr><td align=left><font size=1> &nbsp; &nbsp; &nbsp; &nbsp; <a href="admin.php?ADD=100" target="_parent">TORNA AD AMMINISTRAZIONE</a></font></td><td align=right><font size=1>LIST LOADER- &nbsp; &nbsp; VERSIONE: <?php echo $version ?> &nbsp; &nbsp; BUILD: <?php echo $build ?> &nbsp; &nbsp; </td></tr>
</table>
<?php } ?>
	...
	if ($OK_to_process) {
		...
		if (!eregi(".csv", $leadfile_name) && !eregi(".xls", $leadfile_name)) {
			...
			if (count($field_check)>=5) {
				flush();
				$file=fopen("$lead_file", "r");
				print "<center><font face='arial, helvetica' size=3 color='#009900'><B>Elaborazione in corso$delim_name-delimited file...\n";

			if (strlen($list_id_override)>0) 
				{
				print "<BR><BR>ID LISTA OVERRIDE FOR THIS FILE: $list_id_override<BR><BR>";
				}

			if (strlen($phone_code_override)>0) 
				{
				print "<BR><BR>SOVRASCRITTURA DEL CODICE TELEFONO PER QUESTO FILE: $phone_code_override<BR><BR>";
				}
				...
				print "<center><font face='arial, helvetica' size=3 color='#990000'><B>ERRORE: Il file non ha il numero di campi necessare per essere elaborato.</B></font></center>";
			}
		} else if (!eregi(".csv", $leadfile_name)) {
			# copy($leadfile, "./vicidial_temp_file.xls");
			$file=fopen("$lead_file", "r");

			print "<center><font face='arial, helvetica' size=3 color='#009900'><B>Elaborazione in corsoExcel file... \n";
			if (strlen($list_id_override)>0) 
			{
			print "<BR><BR>ID LISTA OVERRIDE FOR THIS FILE: $list_id_override<BR><BR>\n";
			}
		...
		# print "|$WeBServeRRooT/vicidial/listloader_super.pl $vendor_lead_code_field,$source_id_field,$list_id_field,$phone_code_field,$phone_number_field,$title_field,$first_name_field,$middle_initial_field,$last_name_field,$address1_field,$address2_field,$address3_field,$city_field,$state_field,$province_field,$postal_code_field,$country_code_field,$gender_field,$date_of_birth_field,$alt_phone_field,$email_field,$security_phrase_field,$comments_field,$rank_field,$owner_field, --forcelistid=$list_id_override --lead_file=$lead_file|";
			...
			print "<center><font face='arial, helvetica' size=3 color='#009900'><B>Elaborazione in corsoCSV file... \n";
			if (strlen($list_id_override)>0) 
				{
				print "<BR><BR>ID LISTA OVERRIDE FOR THIS FILE: $list_id_override<BR><BR>";
				}
...
if ($leadfile) {
	...
	if (!eregi(".csv", $leadfile_name) && !eregi(".xls", $leadfile_name)) {
		...
		if (count($field_check)>=5) {
			...
			$file=fopen("$lead_file", "r");
			$total=0; $good=0; $bad=0; $dup=0; $post=0; $phone_list='';
			print "<center><font face='arial, helvetica' size=3 color='#009900'><B>Elaborazione in corso$delim_name-delimited file... ($tab_count|$pipe_count)\n";
			if (strlen($list_id_override)>0) 
			{
			print "<BR><BR>ID LISTA OVERRIDE FOR THIS FILE: $list_id_override<BR><BR>";
			}
	...
	#	echo "|$WeBServeRRooT/vicidial/listloader.pl --forcelistid=$list_id_override --lead-file=$lead_file|";
		...
		print "<script language='JavaScript1.2'>document.forms[0].leadfile.disabled=false; document.forms[0].submit_file.disabled=false; document.forms[0].reload_page.disabled=false;</script>";
			...
			print "    <th align=right><font class='standard' color='white'>VICIDIAL Colonna</font></th>\r\n";
			print "    <th><font class='standard' color='white'>Dati File</font></th>\r\n";
			print "  </tr>\r\n";
			...
			print "    <th colspan=2><input type=submit name='OK_to_process' value='OK PER ELABORARE'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=button onClick=\"javascript:document.location='new_listloader_superL.php'\" value=\"RICOMINCIA\" name='reload_page'></th>\r\n";
			print "  </tr>\r\n";
			print "</table>\r\n";
	# }
		print "<script language='JavaScript1.2'>document.forms[0].leadfile.disabled=false; document.forms[0].submit_file.disabled=false; document.forms[0].reload_page.disabled=false;</script>";
...
if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
	{
	if ($DBX) {print "     Domenica in October to First Domenica in April\n";}
	#**********************************************************************
	# FSO-FSA
	#   2008+ AUSTRALIA ONLY (country code 61)
	#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
	#       Standard time is in effect.
	#     Based on first Domenica in October and first Domenica in April at 1 am.
	#**********************************************************************
...
if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
	{
	if ($DBX) {print "     Last Domenica in September to First Domenica in April\n";}
	#**********************************************************************
	# LSS-FSA
	#   2007+ NEW ZEALAND (country code 64)
	#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
	#       Standard time is in effect.
	#     Based on last Domenica in September and first Domenica in April at 1 am.
	#**********************************************************************
